...
let ori_inst parser =
    ...
    | Some Token.Comma, Some Token.Comma -> (
        match t_token, s_token, i_token with
        | Some Token.Register t, Some Token.Register s, Some Token.Immediate i ->
            let _ = print_imm_opcode parser.addr ~o:0b001101 ~s ~t ~i in
            let _ = Printf.printf " # r%d = r%d | #%d\n" t s i in
            { parser with lexer; addr = parser.addr + 0x4; }
        | _ -> failwith error_msg)
    | _ -> failwith error_msg
...
let sll_inst parser =
    ...
    let lexer, t_token = Lexer.next lexer in
    let lexer, comma_2 = Lexer.next lexer in
    let lexer, a_token = Lexer.next lexer in
    match comma_1, comma_2 with
    | Some Token.Comma, Some Token.Comma -> (
        match d_token, t_token, a_token with
        | Some Token.Register d, Some Token.Register t, Some Token.Immediate a ->
            let _ = print_reg_opcode parser.addr ~f:0b000000 ~s:0 ~t ~d ~a in
            let _ = Printf.printf " # r%d = r%d << #%d\n" d t a in
            { parser with lexer; addr = parser.addr + 0x4; }
        | _ -> failwith error_msg)
    | _ -> failwith error_msg
...
let lhi_inst parser =
    ...
    | Some Token.Comma -> (
        match t_token, i_token with
        | Some Token.Register t, Some Token.Immediate i ->
            let _ = print_imm_opcode parser.addr ~o:0b011001 ~s:0 ~t ~i in
            let _ = Printf.printf " # hi(r%d) = #0x%x\n" t i in
            { parser with lexer; addr = parser.addr + 0x4; }
        | _ -> failwith error_msg)
    | _ -> failwith error_msg
...
let llo_inst parser =
    ...
    | Some Token.Comma -> (
        match t_token, i_token with
        | Some Token.Register t, Some Token.Immediate i ->
            let _ = print_imm_opcode parser.addr ~o:0b011000 ~s:0 ~t ~i in
            let _ = Printf.printf " # lo(r%d) = #0x%x\n" t i in
            { parser with lexer; addr = parser.addr + 0x4; }
        | _ -> failwith error_msg)
    | _ -> failwith error_msg
...
let parse_instruction parser =
    ...
    let rec loop parser =
        ...
        | None ->
            let _ = print_endline "# Generated by my hand-crafted assembler for ECE341" in
            let _ = print_endline "# https://github.com/jalsol/ece341-mips-assembler" in
            parser
    in loop parser
